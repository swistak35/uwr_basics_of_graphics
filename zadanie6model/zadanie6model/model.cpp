...
void Model::MeshEntry::Init(const std::vector<Vertex>& Vertices, const std::vector<unsigned int>& Indices) {
...
bool Model::LoadModel(GLuint programID, const std::string& Filename) {
...
//        std::cout << "v(" << pPos->x << ", " << pPos->y << ", " << pPos->z << ")\n";
...
bool Model::InitMaterials(const aiScene* pScene, const std::string& Filename) {
    ...
    for (unsigned int i = 0 ; i < pScene->mNumMaterials ; i++) {
        ...
        if (!m_Textures[i]) {
            m_Textures[i] = new Texture(GL_TEXTURE_2D, "./white.tga");
